...
// $ses->remove("UserIddrp"); // Không remove vì cần dùng sau này
if($ses->isExpired())
{
...
else
{
    ...
    {
        // Chỉ cho phép xoá đối với nhóm không phải UG004 (user)
        if($loginGrp === "UG004") {
            echo '<script type="text/javascript"> alert("Permission denied."); window.location.href = "view.php"; </script>';
        }
        elseif ($msg == "true") {
            ...
            {
                echo '<script type="text/javascript"> alert("' . $result['message'] . '");window.location.href = "view.php";</script>';
            }
        } else {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (isset($_POST["btnUpdate"])) {
            ...
            $ses->Set("UserIddrp", $targetUserId);
            getTableData($loginGrp, $targetUserId, $db);
            $displaytable = "";
            $disBtnPrint="";
        }
        elseif (isset($_POST["btnPrint"])) {

            $printUserId = ($loginGrp === "UG004") ? $ses->Get("userIdLoged") : $_POST['person'];
            $ses->Set("UserIddrp", $printUserId);
            printData($db);
        }
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <th>'.__('name').'</th>
                                             ...
                                             <th>'.__('paid_by').'</th>
                                             <th>'.__('transaction_mobile_no').'</th>
                                             <th>'.__('amount').'</th>
                                             <th>'.__('remark').'</th>';
            if($logGRP!=="UG004") {
                $GLOBALS['output'].=  '<th>'.__('action').'</th>';
            }else
            {
                $GLOBALS['output'].=' <th>'.__('is_approve').'</th>';
            }
...
function  printData($db)
{
    ...
    if(empty($usId)) {
        $usId = $ses->Get("userIdLoged"); // Fallback to current user
    }
...
// Instanciation of inherited class
    ...
    $billhead ="Thanh toán bởi: ".$GLOBALS["Name"];
    $w = $pdf->GetStringWidth($billhead)+4;
    $pdf->SetLeftMargin(50);
    $pdf->Cell($w,10,$billhead,0,1,'L',true);
    $pdf->Ln(5);
    $pdf->SetX(10);
    $header = array('Ngày thanh toán', 'Thanh toán qua','Số tiền','Ghi chú');
   // $dataall =LoadData($db,$usId);
...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">
            <i class="fa fa-credit-card"></i> Danh sách thanh toán sinh viên
        </h1>
    </div>
</div>
            ...
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form name="apyment" action="view.php"  accept-charset="utf-8" method="post" enctype="multipart/form-data">
                        <button type="submit" class="btn btn-info" style="display:<?php echo $disBtnPrint2;?>;" name="btnPrint" ><i class="fa fa-print"></i><?php echo __('print');?></button>
                    </form>
                    <form name="apyment" action="view.php"  accept-charset="utf-8" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <div>
                                            ...
                                            <button type="submit" class="btn btn-info" style="display:<?php echo $disBtnPrint;?>;" name="btnPrint" ><i class="fa fa-print"></i><?php echo __('print');?></button>
                                        </div>
